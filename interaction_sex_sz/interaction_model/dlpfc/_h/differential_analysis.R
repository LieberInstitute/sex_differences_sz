...
extract_de <- function(contrast, label, efit, feature){
    ...
    top$SE  <- sqrt(efit$s2.post) * pull(as.data.frame(efit$stdev.unscaled), contrast)
...
prep_data <- function(feature){
    counts_lt = list("genes"=here("input/counts/_m",
                     ...
                     "transcripts"=here("input/counts/_m",
                                        "rse_tx.BrainSeq_Phase2_DLPFC.Gencode41.n453.Rdata"),
                     "exons"=here("input/counts/_m",
                                  "rse_exon.BrainSeq_Phase2_DLPFC.Gencode41.n453.Rdata"),
                     "junctions"=here("input/counts/_m",
                                      "rse_jxn.BrainSeq_Phase2_DLPFC.Gencode41.n453.Rdata"))
...
SVA_model <- function(feature){
    x <- memPREP(feature)
    # Design matrix
    mod = model.matrix(~Dx*Sex + Age + mitoRate + rRNA_rate + 
    ...
    colnames(mod) <- gsub("SCZD:Male", "SCZD_Male", colnames(mod))
    colnames(mod) <- gsub("\\(Intercept\\)", "Intercept", colnames(mod))
    # Null model
    null.model = mod |> as.data.frame() |>
        select(-c("Male", "SCZD", "SCZD_Male")) |> as.matrix()
...
groups  <- c('szVctl', 'maleVfemale', 'interaction')
