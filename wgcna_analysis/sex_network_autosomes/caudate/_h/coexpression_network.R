...
prepare_data <- function(){
    suppressMessages(library(dplyr))
                                        # Load sample data
    load("../../../../differential_expression/caudate/_m/genes/voomSVA.RData")
    sample_table = v$design %>% as.data.frame %>% select(-Intercept) %>%
        rename("Sex"="Male") #%>% select(Ancestry, Sex, Age)
                                        # Gene Annotation
    annot_file = paste0("/ceph/projects/v4_phase3_paper/inputs/counts/",
    ...
    fn = paste0("../../../../differential_expression/caudate/_m/",
                "genes/residualized_expression.tsv")
    vsd <- data.table::fread(fn) %>% replace(is.na(.), "") %>%
        filter(V1 %in% annot$names) %>% tibble::column_to_rownames("V1")
    print(dim(vsd))
