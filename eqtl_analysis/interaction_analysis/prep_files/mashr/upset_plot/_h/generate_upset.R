...
generate_upset <- function(feature){
    fn = paste0("../../_m/",feature,"/significant_geneSNP_pairs_3tissues.tsv")
    outfile = paste0("brainseq_e",toupper(feature),"_upsetplot.pdf")
    df = data.table::fread(fn)
                                        # Extract eFeatures by brain region
    caudate = filter(df, Caudate == 1)$`gene_id`
    dlpfc = filter(df, DLPFC == 1)$`gene_id`
    hippo = filter(df, Hippocampus == 1)$`gene_id`
