...
save_results_chunk <- function(m, bhat, shat, Vhat, feature){
    fn = paste0(feature,"/lfsr_allpairs_3tissues.txt.gz")
    chunks = split(1:dim(bhat)[1], cut(1:dim(bhat)[1], 250, labels=FALSE))
    datalist = list()
    total = 10
    for(chunk in names(chunks)){
        ##print(chunk)
        data.chunk = mash_set_data(bhat[chunks[[chunk]],],
                                   shat[chunks[[chunk]],],
                                   V=Vhat)
        m.chunk = mash(data.chunk, g=get_fitted_g(m), fixg=TRUE)
        datalist[[chunk]] <- data.frame(m.chunk$result$lfsr) %>%
            tibble::rownames_to_column("effect") %>%
            tidyr::separate(effect, c("gene_id", "variant_id"),
                            remove=FALSE, sep="_")
    }
    data.table::rbindlist(datalist) %>% data.table::fwrite(fn, sep='\t')
}
...
run_mashr <- function(feature, percentage, SEED){
    ...
    save(m, bhat, shat, Vhat, file=paste0(feature, "/chunk.RData"))
    ## save_results_chunk(m, bhat, shat, Vhat, feature)
}
