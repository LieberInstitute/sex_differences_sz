...
def get_cmd(args, chunk):
    cmd = os.path.join(fastqtl_dir, 'bin', 'fastQTL.static')+' --vcf '+args.vcf\
        +' --bed '+args.bed+' --window '+args.window \
...
parser.add_argument('--phenotype_groups', default='',
                    help='File with mapping of phenotype_id to group_id (gene_id)')
...
parser.add_argument('--window', default='1e6',
                    help='Cis-window size. Default values is 1Mb (1e6).')
...
parser.add_argument("--fastqtl_dir", default="/ceph/opt/enhanced_fastqtl/",
                    help="Location of fastqtl binary")
parser.add_argument('--qvalue_lambda', default=None,
                    help='lambda parameter for pi0est in qvalue.')
parser.add_argument('-t', '--threads', default=8, type=int,
                    help='Number of threads')
parser.add_argument('-o', '--output_dir', default='.', help='Output directory')
args = parser.parse_args()
fastqtl_dir = args.fastqtl_dir
...
with cd(args.output_dir):
    ...
    with tempfile.NamedTemporaryFile(mode='w+') as chunk_list_file, \
        ...
        cmd = 'python3 '+os.path.join(fastqtl_dir, 'python', 'merge_chunks.py') \
            +' {} {} {} --fdr {} -o .'.format(chunk_list_file.name,
                                              log_list_file.name, args.prefix,
                                              args.fdr)
